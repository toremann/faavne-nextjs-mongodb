...
const StocksDates: React.FC<StocksDatesProps> = ({ dividendDate, exDate }) => {
  ...
  return (
    <div className="p-2 my-2 flex flex-col">
      ...
      {dividendDate ? (
        <div className={`flex justify-between m-2 p-2 bg-slate-100 rounded ${isDividendDatePassed ? 'bg-red-500' : 'text-gray-600'} ${isDividendDatePassed && 'text-bold'}`}>
      ...
      {exDate ? (
        <div className={`flex justify-between m-2 p-2 bg-slate-100 rounded ${isExcludingDatePassed ? 'bg-red-500' : 'text-gray-600'}`}>
